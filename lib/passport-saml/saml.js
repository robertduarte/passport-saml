...
SAML.prototype.requestToUrl = function (request, response, operation, additionalParameters, callback) {
  ...
  self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrl - Entrou na função`);

  if (self.options.skipRequestCompression) {

    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrl - Saindo da funcao e entrando na requestToUrlHelper`);
  ...
  function requestToUrlHelper(err, buffer) {
    ...
    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrlHelper - Entrou na funcao`);

    if (err) {

      self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrlHelper - Retornando Callback com erro`);
    ...
    if (operation === 'logout') {
      if (self.options.logoutUrl) {
        ...
        self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrlHelper - Operação é logout`);

        target = url.parse(self.options.logoutUrl, true);
      }
    } else if (operation !== 'authorize') {

      self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrlHelper - Operacao desconhecida, enviando callback com erro`);
    ...
    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - requestToUrlHelper - Enviando callback`);
...
SAML.prototype.getAuthorizeUrl = function (req, callback) {
  ...
  self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeUrl - Entrou na funcao`);
  self.generateAuthorizeRequest(req, { isPassive: self.options.passive, isHttpPostBinding: false }, function(err, request) {
    if (err) {

    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeUrl - Retornando callback com erro: " + ${err}`);

      return callback(err);
    }
    var operation = 'authorize';

    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeUrl - Saindo da funcao e entrando na requestToUrl`);
...
SAML.prototype.getAuthorizeForm = function (req, callback) {
  ...
  self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeForm - Entrou na funcao`);

  self._functionLogger.info(`PASSPORT - PID - ${process.pid}, Req Query Login AppVersion: ${req && req.query ? req.query : "NotFound"}`);
  self._functionLogger.info(`PASSPORT - PID - ${process.pid}, Req Body Login AppVersion: ${req && req.body ? req.body : "NotFound"}`);
  var appVersionLog = req && req.query && req.query.AppVersion ? req.query.AppVersion : req && req.body && req.body.AppVersion ? req.body.AppVersion : "none";
  var queryParams = req && req.query ? req.query : undefined;
  // The quoteattr() function is used in a context, where the result will not be evaluated by javascript
  // but must be interpreted by an XML or HTML parser, and it must absolutely avoid breaking the syntax
  // of an element attribute.

  self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeForm - Acabou a funcao`);
  ...
  var getAuthorizeFormHelper = function(err, buffer) {
    ...
    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeFormHelper - Entrou na funcao`);

    if (err) {

      self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeForm - Erro: ${err}`);
    ...
    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - getAuthorizeForm - Chamando Callback`);
  ...
  self.generateAuthorizeRequest(req, { isPassive: self.options.passive, isHttpPostBinding: true }, function(err, request) {
    ...
    self._functionLogger.info(`PASSPORT - PID: ${process.pid} - generateAuthorizeRequest - Entrou na funcao`);
    
    if (err) {

      self._functionLogger.info(`PASSPORT - PID: ${process.pid} - generateAuthorizeRequest - Erro: ${err}`);

      return callback(err);
    }

    self._functionLogger.info("SAMLRequest: ",request);

    if (self._functionSaveSamlData) {

      self._functionLogger.info(`PASSPORT - PID: ${process.pid} - generateAuthorizeRequest - Chamando SaveSamlData`);

      self._functionSaveSamlData({ SAMLRequest: { xml: request, appVersion: appVersionLog, query: queryParams } }, 'Login');
    }
...
SAML.prototype.getLogoutUrl = function(req, callback) {
  ...
  if (this._functionSaveSamlData) {
    this._functionSaveSamlData({ SamlLogoutRequest: request }, 'Logout');
  }
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  if (self._functionSaveSamlData) {
    self._functionSaveSamlData({ SAMLResponse: xml.toString() });
  }
...
SAML.prototype.validatePostRequest = function (container, callback) {
  ...
  self._functionLogger.info("SAML Strategy - SAMLRequest: ",xml);
