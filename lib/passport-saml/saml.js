...
SAML.prototype.getAuthorizeForm = function (req, callback) {
  ...
  self.generateAuthorizeRequest(req, { isPassive: self.options.passive, isHttpPostBinding: true }, function(err, request) {
    ...
    console.log("SAMLRequest:",request);
    if (self._functionSaveSamlData) {
      self._functionSaveSamlData({ SAMLRequest: { xml: request, appVersion: req && req.query && req.query.AppVersion ? req.query.AppVersion : undefined } }, 'Login');
    }
...
SAML.prototype.validatePostRequest = function (container, callback) {
  ...
  console.log("SAML Strategy - SAMLRequest:",xml);
